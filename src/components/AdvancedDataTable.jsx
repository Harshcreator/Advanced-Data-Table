...
const betweenFilterFn = (rows, columnId, filterValue) => {
    if (!Array.isArray(filterValue) || filterValue.length !== 2) {
      return rows;
    }
    const [min, max] = filterValue;
    return rows.filter(row => {
      const value = row.original[columnId]; // Accessing the original data
      return value != null && value >= min && value <= max; // Ensure value is not null or undefined
    });
  };
...
const AdvancedDataTable = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        accessorKey: 'name',
        header: 'Name',
        filterFn: (rows, columnId, filterValue) => fuzzyFilter(rows, columnId, filterValue),
      },
      {
  ...
  return (
    <div style={{ padding: '1rem', position: isFullScreen ? 'fixed' : 'static', top: 0, left: 0, right: 0, bottom: 0, zIndex: 1000 }}>
      <MaterialReactTable
        columns={columns}
        data={data}
        initialState={{ columnVisibility, globalFilter }}
        ...
        renderTopToolbarCustomActions={() => (
          <Box display="flex" gap="1rem" alignItems="center">
            <Button onClick={() => setIsFullScreen(!isFullScreen)}>{isFullScreen ? 'Exit Full Screen' : 'Full Screen'}</Button>
            <Button onClick={() => setColumnVisibility({})}>Reset Column Visibility</Button>
            <TextField
              value={globalFilter}
